...
class Section {
     ...
     * @var {number} index - this is a section number that we receive in format "list[0](left), formats[1](right)"
     ...
     * @const {number} id - for get the element "key" at context (array[id])*/
    ...
    /**
     * Intend obtain escential data to building of the queries que present iteractivity to user, call data according at context
     * @param {string} currentSection - Correspond to the name of current section, its clicked by user
     * @return {object} get a object with propierties like indexSection, containers of the section in context; collections, each corresponding to a container asignated, through get the query and fill containers in the section
    ...
    /**
     * This method allows us to configure the request that we send to Firebase method "getDocs()" with which we obtain the data map "documentSnapshot" or "querySnapshot"
     * @param {number} indexSection - Is the index of the current section, with this select a default query in case of dont receive a configQuery (handler)
     * @param {string} collectionToSearch - Contain name of the collection to query in database, with this name we can inspect the keys of object "metaData" to get static data customized (example: icon)
     ...
     * @return {object} we get a object with keys {icon, where, pagination} this for the config of the querySnapshot request by (getDocs) through at "query()" with a config specific; the above methods belongs to the backend of firebase
     */
    ...
    /**
     * through param "context" that represent the index of the current section clicked by the user (main navbar)
